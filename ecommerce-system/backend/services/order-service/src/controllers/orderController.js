...
class OrderController {
  ...
  async getOrdersOverview(req, res) {
    try {
      ...
      const paidOrders = await Order.count({ where: { ...where, status: 'PAID' } });
    ...
    try {
      ...
      if (order.payment_status !== 'PAID') {
        return res.status(400).json({
          success: false,
          error: {
            message: '只有已付款的訂單才能退款'
          }
        });
      }

      // 退款訂單
      await order.updateStatus('REFUNDED', { reason });

      // 重新查詢訂單
      const refundedOrder = await Order.findByPk(id, {
        include: [{
          model: OrderItem,
          as: 'orderItems',
          required: false
